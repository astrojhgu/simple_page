...
where F: FnOnce(&mut web::ServiceConfig){
    ...
    pub fn with_static_file(self, web_path: String, file_path: std::path::PathBuf)->CfgChain<impl FnOnce(&mut web::ServiceConfig)>{
        let f:Box<_>=Box::new(move |s: &mut web::ServiceConfig|{
            s.route(web_path.clone().as_str(), web::get().to(move ||{
                NamedFile::open(file_path.clone())
            }));
        });
        self.add_cfg(f)
    }
...
pub fn article_handler(cfg: &DynArticleCfg, path: web::Path<String>) -> Either<HttpResponse, NamedFile> {
    ...
    if path.ends_with(".md") {
        if let Result::Ok(s) = read_to_string(article_path) {
            ...
            if let Ok(s) = cfg
                .article_template
                .as_ref()
                .render("article_template", &json!({ "content": html_output , "parent_link": parent_link}))
            {
                //println!("{}", s);
                Either::A(
                    HttpResponse::Ok()
        ...
        if let Ok(s) = cfg
            .article_template
            .as_ref()
            .render("dir_template", &json!({ "items": titles , "parent_link": parent_link}))
        {
            //println!("{:?}", s);
            Either::A(
                HttpResponse::Ok()
...
pub fn cfg_server(article_cfgs: Vec<Box<DynArticleCfg>>,
                static_cfgs: Vec<Box<StaticCfg>>,
                srv_cfg: &mut web::ServiceConfig){
    for ac in article_cfgs{
        let cfg1=ac.clone();
        srv_cfg.route((ac.article_prefix.clone() + "/{tail:.*}").as_str(),
        web::get().to(move |x: web::Path<String>| {
        article_handler(&cfg1, x)
        }
        ));
    }
    ...
    for sc in static_cfgs{
        let cfg1=sc.clone();
        srv_cfg
        .route((sc.static_prefix.clone() + "/{tail:.*}").as_str(), web::get().to(
            move |x: web::Path<String>|{
                static_handler(&cfg1, x)
            }
        ))
        ;
    }
...
pub fn compose_cfg(article_cfg: Vec<Box<DynArticleCfg>>, 
                    static_cfg: Vec<Box<StaticCfg>>,
...
pub fn compose_cfg1(article_cfg: Vec<Box<DynArticleCfg>>,
                    static_cfg: Vec<Box<StaticCfg>>,
