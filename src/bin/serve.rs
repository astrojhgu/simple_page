...
fn main() {
    let matches=clap::App::new("server")
        ...
        .arg(clap::Arg::with_name("data_path")
            .required(true)
            .takes_value(true)
    ...
    let article_cfg = vec![Box::new(simple_page::DynArticleCfg::new(
                        article_path.into(),
                        article_template.clone().into(),
                        dir_template.clone().into(),
                        "articles".into()
    )),
    Box::new(simple_page::DynArticleCfg::new(
    ...
    let static_cfg=Box::new(simple_page::StaticCfg::new("static".to_string(),                      static_dir.into(),
    ));
    ...
    HttpServer::new(move || {
        ...
        App::new()
            .configure(
                ...
                .with_static_file("/favicon.ico".into(), (root_path.clone()+"/favicon.ico").into())
                .with_static_file("/robots.txt".into(), (root_path.clone()+"/robots.txt").into())
                .cfg_fn
            )
            .wrap(Logger::default())
